    ...
    if stopall:
        try:
            if pipes["mpconnector"]:
                ...
                mpp["mpconnector"].join(timeout=3)
                if not mpp_is_alive(mpp, "mpconnector"):
                    logger.debug(whoami() + "mpconnector joined!")
                else:
                    logger.debug(whoami() + "mpconnector not joined, killing now ...")
                    kill_mpp(mpp, "mpconnector", timeout=3)
                    logger.debug(whoami() + "... mpconnector killing done!")
...
def ginzi_main(cfg_file, cfg, dirs, subdirs, guiport, mp_loggerqueue):
    ...
    mpp_renamer = mp.Process(target=renamer.renamer, args=(renamer_child_pipe, renamer_result_queue, mp_loggerqueue, filewrite_lock, ))
    mpp_renamer.start()
    mpp["renamer"] = mpp_renamer

    # start mpconnector
    logger.info(whoami() + "starting mpconnector process ...")
    mpp_connector = mp.Process(target=mpconnections.mpconnector, args=(mpconnector_child_pipe, cfg, server_ts, mp_loggerqueue, ))
    mpp_connector.start()
        ...
        while not event_stopped.is_set():
            ...
            if not dl:
                ...
                if nzbname:
                    ...
                    dl = downloader.Downloader(cfg, dirs, ct, mp_work_queue, articlequeue, resultqueue, mpp, pipes,
