...
class Downloader(Thread):
    ...
    def postprocess_nzb(self, nzbname, downloaddata0):
        ...
        self.guiconnector.set_data(downloaddata, self.ct.threads, self.ct.servers.server_config, "postprocessing", self.serverhealth())
        ...
        if self.pwdb.exc("db_nzb_getstatus", [nzbname], {}) == -4:
            ...
            self.guiconnector.set_data(downloaddata, self.ct.threads, self.ct.servers.server_config, "failed", self.serverhealth())
            return -1
        else:
            self.logger.info("Copy/Move of NZB " + nzbname + " success!")
            self.guiconnector.set_data(downloaddata, self.ct.threads, self.ct.servers.server_config, "success", self.serverhealth())
            self.pwdb.exc("db_nzb_update_status", [nzbname, 4], {})
    ...
    def run(self):
        ...
        while not self.stopped:
            ...
            #                               self.ct.servers.server_config, "downloading", self.serverhealth())
            #except Exception as e:
            ...
            if self.connection_health < 0.65:
                ...
                return_reason = "connection restart"
                self.results = nzbname, ((bytescount0, availmem0, avgmiblist, filetypecounter, nzbname, article_health,
                                          overall_size, already_downloaded_size, p2, overall_size_wparvol, allfileslist)), return_reason, self.main_dir
                self.stop()
                continue
