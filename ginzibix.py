    ...
    def postprocess_nzb(self, nzbname, downloaddata):
        ...
        if ispw:
            if self.pwdb.db_nzb_get_password(nzbname) == "N/A":
                ...
                pw = lib.get_password(self.verifiedrar_dir, self.pw_file, nzbname, self.logger)
                if pw:
...
def main(cfg, pwdb, logger):
    # loop;
    #      sighandler.mppid["nzbname"]["unrarer"]
    #      wait for next nzb0
    #      start threads
    #      download
    #      postprocess + download next nzb
    ...
    mpp_nzbparser = mp.Process(target=lib.ParseNZB, args=(pwdb, dirs["nzb"], logger, ))
    mpp_nzbparser.start()
    mpp_pid["nzbparser"] = mpp_nzbparser.pid

    # start decoder mpp
    logger.debug("Starting decoder process ...")
    mpp_decoder = mp.Process(target=lib.decode_articles, args=(mp_work_queue, pwdb, logger, ))
    mpp_decoder.start()
...
if __name__ == '__main__':
    ...
    main(cfg, pwdb, logger)
    # main0(cfg, None, pwdb, 0, logger)
